    ...
    ER: RaftEngine,
{
    ...
    pub fn switch_replication_mode(&mut self, state: &Mutex<GlobalReplicationState>) {
        ...
        let (enable_group_commit, calculate_group_id) = if guard.status().get_mode() == ReplicationMode::Majority {
            self.replication_mode_version = 0;
            self.dr_auto_sync_state = DrAutoSyncState::Async;
            (false, false)
        } else {
    ...
    /// If enable_group_commit or recalculate_group_id is true, it should calculate group id for dr-auto-sync.
    /// When state is sync-recover, enable_group_commit is set to false, recalculate_group_id is set to true.
