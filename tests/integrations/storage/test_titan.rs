...
fn test_delete_files_in_range_for_titan() {
    ...
    let resource = cfg.rocksdb.build_resources((Some(Default::default()),Some(Default::default())));
    let kv_db_opts = cfg.rocksdb.build_opt(&resource, cfg.storage.engine, false);
